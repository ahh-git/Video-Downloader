...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS users 
                 (email TEXT PRIMARY KEY, name TEXT, photo TEXT, joined_at TEXT, is_banned INTEGER)''')
...
def add_user(email, name, photo):
    ...
    try:
        conn.execute("INSERT OR IGNORE INTO users VALUES (?, ?, ?, ?, ?)", 
                  (email, name, photo, datetime.now(), 0))
        conn.commit()
    except: pass
    conn.close()
...
def get_daily_downloads():
    ...
    try:
        ...
        query = f"SELECT timestamp FROM history"
...
def get_top_users():
    conn = get_connection()
    try:
        query = "SELECT email, count(*) as count FROM history GROUP BY email ORDER BY count DESC LIMIT 5"
        df = pd.read_sql_query(query, conn)
        return df
    except: return pd.DataFrame()
    conn.close()
