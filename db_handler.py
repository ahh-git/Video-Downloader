...
DB_NAME = "unisaver_ultimate_v4.db"
...
def add_user(email, name, photo):
    conn = get_connection()
    try:
        # Check if user exists to preserve 'joined_at'
        exists = conn.execute("SELECT count(*) FROM users WHERE email = ?", (email,)).fetchone()[0]
        if exists == 0:
            conn.execute("INSERT INTO users (email, name, photo, joined_at, is_banned) VALUES (?, ?, ?, ?, ?)", 
                      (email, name, photo, datetime.now().strftime("%Y-%m-%d"), 0))
            conn.commit()
    except: pass
    conn.close()
...
def get_user_details(email):
    conn = get_connection()
    try:
        row = conn.execute("SELECT joined_at, is_banned FROM users WHERE email = ?", (email,)).fetchone()
